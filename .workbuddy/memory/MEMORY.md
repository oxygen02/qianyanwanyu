...
| 文件 | 说明 |
|------|------|
| `utils/content-cleaner.js` | 智能内容清洗：URL识别、广告过滤、结构化分块、页面序列构建 |
| `engine/image-page-renderer.js` | 图片插画页渲染：纸张背景、图片居中、阴影边框、说明文字 |
| `cloud/functions/fetchArticle/index.js` | 文章抓取云函数：文本密度算法提取正文、图片过滤、编码自动检测 |
| `cloud/functions/fetchArticle/package.json` | 云函数配置 |

### 重写文件

| 文件 | 变更 |
|------|------|
| `pages/shiwen/shiwen.js` | 支持网址自动提取/纯文本清洗、页面序列管理、滑动翻页、多页导出 |
| `pages/shiwen/shiwen.wxml` | 4状态切换、文章信息栏、触摸翻页容器、页码指示器 |
| `pages/shiwen/shiwen.wxss` | 加载动画、书页阴影、翻页提示、页码圆点样式 |
...
用户输入（网址/纯文本）
    │
    ├─ 网址 → 云函数 fetchArticle → 提取正文+图片 → 返回结构化数据
    └─ 纯文本 → content-cleaner.js → 智能清洗 → 结构化分块
    │
    ▼
buildPageSequence() → 文字页 + 图片插画页 交错排列
    │
    ├─ 文字页 → renderer.js → 排版+油墨渲染
    └─ 图片页 → image-page-renderer.js → 全页居中展示
    │
    ▼
左右滑动/点击翻页 → 逐页浏览 → 导出保存
```
...
- **广告清洗**：7种模式，只在文章后60%匹配，避免误删正文
- **图片过滤**：尺寸<80px跳过、URL含ad/banner/logo跳过
- **翻页交互**：滑动阈值50px + 点击左1/3/右1/3区域
- **云函数**：不依赖cheerio，原生正则+文本密度算法，减少依赖体积
